...
- **Protocol Parser / 协议解析**: Parses 110-field measurement frames (`M`) and 6-field RTC time-ack frames (`T`). Strict field validation, JSON escaping for arbitrary text. Generates structured JSON with bridge/source/channel/protocol metadata and schema_version. / 解析 110 字段测量帧（`M`）和 6 字段时间应答帧（`T`），严格字段校验，自由文本 JSON 转义，生成带元信息和 schema_version 的结构化 JSON。
...
- **GUI Status Uplink / GUI 状态上报**: Publishes ESP bridge status JSON to `pulseox/status` with MQTT retained/LWT support, and sends the same `esp_status` JSON over active USB GUI sessions. / ESP 主动上报 USB、WiFi、MQTT、UART、STM32 协议状态，供上位机 GUI 直接显示连接健康。
...
│   ├── app_main.c           # app_main entry point
...
| `MQTT_STATUS_TOPIC` | `"pulseox/status"` | Retained ESP bridge status topic for GUI |
...
The STM32 outputs a 110-column CSV frame with the following fields in order:
...
| 102-109 | ecg quality | uint | ECG signal quality, DSP metrics, DMA watermark |

Frames that are not exactly 110 CSV fields are still forwarded as JSON, but
`parse_ok` is `false` and `parse_warnings` includes
`field_count_<n>_expected_110`. Legacy 102-field (v2) frames are parsed
correctly but marked as schema-mismatch with a warning; they do not produce
ECG quality fields.
...
The STM32 responds with a time-ack frame (`T`).
...
`tools/publish_pc_time.py` — Publishes the local PC system time to the MQTT command topic so the STM32 can sync its RTC. The payload is `SETTIME YYYY-MM-DD HH:MM:SS`.
